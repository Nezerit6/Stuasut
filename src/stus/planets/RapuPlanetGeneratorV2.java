...
public class RapuPlanetGeneratorV2 extends PlanetGenerator{
    ...
    Block[][] arr =
            {
                    {StuasutBlocks.gert, StuasutBlocks.gert, Blocks.stone, StuasutBlocks.gert, Blocks.stone, Blocks.stone},
                    {StuasutBlocks.limestone, StuasutBlocks.gert, StuasutBlocks.limestone, StuasutBlocks.limestone, StuasutBlocks.gert, StuasutBlocks.gert},
                    {StuasutBlocks.mercurymud, Blocks.stone, StuasutBlocks.mercurymud, Blocks.stone, StuasutBlocks.mercurymud, StuasutBlocks.mercurymud},
                    {StuasutBlocks.gert, StuasutBlocks.gert, StuasutBlocks.slate, StuasutBlocks.slate, StuasutBlocks.slate, StuasutBlocks.gert}
            };
    {
    ...
    ObjectMap<Block, Block> diethyl = ObjectMap.of(
            StuasutBlocks.gert, StuasutBlocks.limestone
    );
    ObjectMap<Block, Block> gallium = ObjectMap.of(
            StuasutBlocks.gert, StuasutBlocks.mercurymud
    );
            ...
            for(Ptile other : tile.tiles){
                ...
                if(
                        osec.id == sector.planet.startSector || //near starting sector
                                osec.generateEnemyBase && poles < 0.86 || //near other base
    ...
    protected void generate(){
                        ...
                        for(int y = -rad; y <= rad; y++){
                            ...
                            if(Structs.inBounds(wx, wy, width, height) && Mathf.within(x, y, rad)){
                                ...
                                if(Mathf.within(x, y, rad - 1) && !other.floor().isLiquid){
                                    ...
                                    other.setFloor((Floor)(floor == StuasutBlocks.gert || floor == StuasutBlocks.limestone ? StuasutBlocks.limestone : StuasutBlocks.diethylEtherPuddle));
                                }
                            }
                        }
                    }
        ...
        pass((x, y) -> {
            ...
            if(value > 0.17f && !Mathf.within(x, y, fspawn.x, fspawn.y, 12 + rrscl)){
                ...
                if(!(floor == StuasutBlocks.mercurymud || floor == StuasutBlocks.slate || floor == Blocks.snow || floor.asFloor().isLiquid)){
                    floor = spore ?
                            (deep ? StuasutBlocks.diethylEtherPuddle : StuasutBlocks.limestone) :
                            (deep ? StuasutBlocks.diethylEtherPuddle :
                                    (floor == StuasutBlocks.gert || floor == StuasutBlocks.limestone ? StuasutBlocks.limestone : StuasutBlocks.diethylEtherPuddle));
                }
            }
        });
        ...
        pass((x, y) -> {
            ...
            if(floor.asFloor().isLiquid && floor.asFloor().shallow){
                ...
                floor = floor == StuasutBlocks.mercurymud ? StuasutBlocks.gert : StuasutBlocks.galliumPuddle;
            }
        });
        ...
        pass((x, y) -> {
            //hotrock tweaks
            if(floor == StuasutBlocks.slate){
                if(Math.abs(0.5f - noise(x - 90, y, 4, 0.8, 80)) > 0.035){
                    floor = StuasutBlocks.gert;
                }else{
                    ore = Blocks.air;
                    boolean all = true;
                    for(Point2 p : Geometry.d4){
                        Tile other = tiles.get(x + p.x, y + p.y);
                        if(other == null || (other.floor() != StuasutBlocks.slate && other.floor() != StuasutBlocks.slate)){
                            all = false;
                        }
                    }
                    if(all){
                        floor = StuasutBlocks.slate;
                    }
                }
            }else if(genLakes && floor != StuasutBlocks.gert && floor != StuasutBlocks.mercurymud && floor.asFloor().hasSurface()){
                float noise = noise(x + 782, y, 5, 0.75f, 260f, 1f);
                if(noise > 0.67f && !roomseq.contains(e -> Mathf.within(x, y, e.x, e.y, 14))){
                    if(noise > 0.72f){
                        floor = noise > 0.78f ? StuasutBlocks.galliumPuddle : (floor == StuasutBlocks.gert ? StuasutBlocks.mercurymud : StuasutBlocks.limestone);
                    }else{
                        floor = (floor == StuasutBlocks.gert ? floor : StuasutBlocks.mercurymud);
                    }
                }
            }
        ...
        if(ruinCount > 0){
            ...
            for(int i = 0; i < ints.size && placed < ruinCount; i++){
                ...
                if(part != null && RapuBase.tryPlace(part, x, y, Team.derelict, (cx, cy) -> {
                    Tile other = tiles.getn(cx, cy);
                    if(other.floor().hasSurface()){
                        other.setOverlay(StuasutBlocks.oreBarium);
                        for(int j = 1; j <= 2; j++){
                            for(Point2 p : Geometry.d8){
                                Tile t = tiles.get(cx + p.x*j, cy + p.y*j);
                                if(t != null && t.floor().hasSurface() && rand.chance(j == 1 ? 0.4 : 0.2)){
                                    t.setOverlay(StuasutBlocks.oreBarium);
                                }
                            }
                        }
                    }
        ...
        state.rules.spawns = RapuWaves.generate(sector.threat, new Rand(), state.rules.attackMode, state.rules.attackMode && spawner.countGroundSpawns() == 1, naval);
        ...
        state.rules.spawns = RapuWaves.generate(difficulty, new Rand(sector.id), state.rules.attackMode, state.rules.attackMode && spawner.countGroundSpawns() == 0, naval);
    }
